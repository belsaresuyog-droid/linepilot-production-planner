...
export default function Home() {
  ...
  const [tab, setTab] = useState<"plan" | "feeder" | "actual" | "schedule" | "capacity" | "catalog" | "skills" | "users" | "twin">("plan");
  ...
  const [userDraft, setUserDraft] = useState({ name: "", email: "", role: "user" as "admin" | "user" });
  ...
  useEffect(() => { fetch("/api/auth/session").then((response) => response.ok ? response.json() : { user: null, isAdmin: false }).then((payload: { user?: AuthUser | null; isAdmin?: boolean }) => { setAuthUser(payload.user ?? null); setAuthIsAdmin(Boolean(payload.isAdmin)); setAuthChecked(true); }).catch(() => { setAuthUser(null); setAuthIsAdmin(false); setAuthChecked(true); }); }, []);
